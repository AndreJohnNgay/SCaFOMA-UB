...
function ViewConcessionaire() {
    ...
    const concessionaire = location.state
        ?.concessionaire;
    ...
    const filteredSalesData = sampleSalesData.filter((sale) => sale.date.toLowerCase().includes(searchTerm.toLowerCase()) || sale.time.toLowerCase().includes(searchTerm.toLowerCase()) || sale.soldTo.toLowerCase().includes(searchTerm.toLowerCase()) || sale.item.toLowerCase().includes(searchTerm.toLowerCase()) || sale.totalPrice.toString().includes(searchTerm));
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-md w-3/4 mt-6">
                            ...
                            <tbody>
                                {currentRows.map((sale) => (
                                    <tr key={sale.id}>
                                        ...
                                        <td className="border-t py-2 px-4">₱{sale
                                                .totalPrice
                                                .toFixed(2)}</td>
                                    </tr>
                                ))}
                        ...
                        <div className="mt-4 flex justify-center">
                            {Array.from({
                                ...
                                <button
                                    key={index}
                                    className={`px-3 py-1 border rounded mx-1 ${currentPage === index + 1
