...
const cafeteriaData = [
    { id: 1, name: 'Cafeteria One', location: 'Building F' },
    { id: 2, name: 'Cafeteria Two', location: 'Building H' },
    { id: 3, name: 'Cafeteria Three', location: 'Building A' },
];
...
function Cafeterias() {
    ...
    const [editCafeteria, setEditCafeteria] = useState(null);
    const [isEditing, setIsEditing] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const rowsPerPage = 10; // Show 10 rows per page

    const filteredCafeterias = cafeteriaData.filter(cafeteria =>
    ...
    const handleEditCafeteria = () => {
        // Logic to edit cafeteria details (not implemented)
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button
                            onClick={() => {
                                setModalOpen(true);
                                setIsEditing(false);
                                setEditCafeteria({ name: '', location: '' }); // Reset for new entry
                            }}
                            className="bg-red-950 text-white px-4 py-2 rounded hover:bg-red-600 flex items-center"
                        >
                            <i className="fas fa-plus mr-2"></i> {/* Add icon */}
                            Add Cafeteria
                        </button>
                    </div>
                                ...
                                <tr key={cafeteria.id}>
                                    ...
                                    <td className="border-t py-2 px-4 flex space-x-2">
                                        <button
                                            ...
                                            className="bg-red-950 text-white px-4 py-2 rounded hover:bg-red-600 flex items-center"
                                        >
                                            <i className="fas fa-eye mr-2"></i> {/* View icon */}
                                            View
                                        </button>
                                        <button
                                            onClick={() => {
                                            ...
                                            className="bg-red-950 text-white px-4 py-2 rounded hover:bg-red-600 flex items-center"
                                        >
                                            <i className="fas fa-edit mr-2"></i> {/* Edit icon */}
                                            Edit
                                        </button>
                                        <button
                                            onClick={() => {
                                                // Logic to delete cafeteria (not implemented)
                                                console.log(`Deleted cafeteria: ${cafeteria.name}`);
                                            }}
                                            className="bg-red-950 text-white px-4 py-2 rounded hover:bg-red-600 flex items-center"
                                        >
                                            <i className="fas fa-trash-alt mr-2"></i> {/* Delete icon */}
                                            Delete
                                        </button>
                                    </td>
                                </tr>
                    ...
                    {modalOpen && (
                        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                            <div className="bg-white p-6 rounded shadow-lg">
                                <h2 className="text-lg font-semibold mb-4">{isEditing ? 'Edit Cafeteria' : 'Add Cafeteria'}</h2>
                                <input
                                    type="text"
                                    placeholder="Cafeteria Name"
                                    value={editCafeteria?.name || ''}
                                    onChange={(e) => setEditCafeteria({ ...editCafeteria, name: e.target.value })}
                                    className="mb-2 p-2 border border-gray-300 rounded w-full"
                                />
                                <input
                                    type="text"
                                    placeholder="Location"
                                    value={editCafeteria?.location || ''}
                                    onChange={(e) => setEditCafeteria({ ...editCafeteria, location: e.target.value })}
