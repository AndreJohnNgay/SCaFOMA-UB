...
const concessionaireData = [
    { id: 1, name: 'John Doe', email: 'john@example.com', concessionName: 'Burger Shack' },
    { id: 2, name: 'Jane Smith', email: 'jane@example.com', concessionName: 'Pizza Corner' },
    { id: 3, name: 'Alice Johnson', email: 'alice@example.com', concessionName: 'Sushi Spot' },
    //... more data
];
...
function Concessionaires() {
    ...
    const [editConcessionaire, setEditConcessionaire] = useState(null);
    const [confirmDelete, setConfirmDelete] = useState(false);
    const [selectedConcessionaire, setSelectedConcessionaire] = useState(null);

    const filteredConcessionaires = concessionaireData.filter(concessionaire =>
        concessionaire.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <div className="flex-1 flex flex-col">
                ...
                <div className="p-6">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        >
                            <i className="fas fa-plus mr-2"></i> Add Concessionaire
                        </button>
                    </div>
                            ...
                            {currentRows.map((concessionaire) => (
                                <tr key={concessionaire.id}>
                                    ...
                                    <td className="border-t py-2 px-4 flex space-x-2">
                                        <button
                                            onClick={() => navigate('/view-concessionaire', { state: { concessionaire } })}
                                            className="bg-red-950 text-white px-4 py-2 rounded hover:bg-red-600"
                                        >
            ...
            {modalOpen && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded shadow-lg">
                        <h2 className="text-lg font-semibold mb-4">{isEditing ? 'Edit Concessionaire' : 'Add Concessionaire'}</h2>
                        <input
                            type="text"
                            placeholder="Name"
                            value={editConcessionaire?.name || ''}
                            onChange={(e) => setEditConcessionaire({ ...editConcessionaire, name: e.target.value })}
                            className="mb-2 p-2 border border-gray-300 rounded w-full"
                        />
                        <input
                            type="email"
                            placeholder="Email"
                            value={editConcessionaire?.email || ''}
                            onChange={(e) => setEditConcessionaire({ ...editConcessionaire, email: e.target.value })}
                            className="mb-2 p-2 border border-gray-300 rounded w-full"
                        />
                        <select
                            value={editConcessionaire?.concessionName || ''}
                            onChange={(e) => setEditConcessionaire({ ...editConcessionaire, concessionName: e.target.value })}
            ...
            {confirmDelete && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded shadow-lg">
                        ...
                        <p>Are you sure you want to delete {selectedConcessionaire?.name}?</p>
                        <div className="flex justify-end mt-4">
                            <button
                                onClick={() => handleDelete(selectedConcessionaire?.id)}
                                className="bg-red-950 text-white px-4 py-2 rounded hover:bg-red-600 mr-2"
                            >
                                Yes, Delete
                            </button>
