...
const AddItemVariationsScreen = ({ navigation, route }) => {
  ...
  const [variations, setVariations] = useState([{ variation: '', sizes: updatedItem.sizes.map(size => ({ ...size })) }]);

  const handleAddVariation = () => {
    setVariations([...variations, { variation: '', sizes: updatedItem.sizes.map(size => ({ ...size })) }]);
  };
  ...
  const handleNext = () => {
    ...
    navigation.navigate('AddItemAddOns', { updatedItem: updatedItemWithVariations, onAddItem });
  };
  ...
  const renderSizePriceItem = ({ item, index }, variationIndex) => (
    <View key={index} style={styles.sizePriceContainer}>
      ...
      <TextInput
        placeholder="₱ Price"
        value={item.price}
        onChangeText={(value) => handlePriceChange(variationIndex, index, value)}
